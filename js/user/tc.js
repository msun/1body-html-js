var TC_CONF = TC_CONF || {
    // TC_CONF_SECTION
    "activityLevel":"FULL","thirdPartyCookie":true,"trackingTime":31536000000
...
var TC = TC || (function() {
    ...
    var TCT = {
        ...
        collector: function() {
            return "us-west-2-qa-tc.lift.acquia.com/tc";
        },

        generator: function() {
            return "us-west-2-qa-tc.lift.acquia.com/tc3ptidgen";
        },

        doNotTrackController: function() {
            return "us-west-2-qa-tc.lift.acquia.com/tc3dnt";
        }
    ...
    var Queue = {
        ...
        getNext: function() {
            this._cookie.load();
            var value = this._cookie.getValue();
            if ( !value ) { // no queue, no next queue url
                return;
            }
            else {
        ...
        if ( Config.isActivityLevelCapture() ) {
            ...
            if ( BOM.isSupported() && Config.isActivityLevelFull()) {
                ...
                Config.setListener( function ( params ) {
                    ...
                    s.async = true;
                    s.src = ('https:' == document.location.protocol ? 'https' : 'http') + '://'+'qa-cdn.lift.acquia.com/'+Config.account+'/'+jsFileToLoad;
                    var x = document.getElementsByTagName('script')[0];
